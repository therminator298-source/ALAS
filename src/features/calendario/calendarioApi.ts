...
export async function listTareas(fromISO: string, toISO: string, deposito?: string): Promise<{ rows: Tarea[]; live: boolean }> {
  ...
  let q = supabaseCal
    .from('tareas')
    .select('id,titulo,descripcion,fecha,hora,responsable,deposito,prioridad,estado,usuario,orden,created_at')
    .gte('fecha', fromISO).lte('fecha', toISO);
  if (deposito) q = q.eq('deposito', deposito);
  const { data, error } = await q.order('orden', { nullsFirst: false }).order('fecha').order('hora', { nullsFirst: true });
